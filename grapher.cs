...
namespace _2020_07_26_Console_Grapher
{
    public class grapher : Form
    {
        ...
        public grapher(string arguments)
        {
        ...
        public SKPoint[] coordinates()
        {
            ...
            SKPoint[] coordinates_list = new SKPoint[length_of_coordinate_array];
            ...
            try
            {
                ...
                for(int i = 0; i < coordinates_list.GetLength(0); i++)
                {
                        coordinates_list[i] = new SKPoint(x, -1 * (float)numbers[0].GetValue(i));
                        x++;
                }
            }
            catch
            {
                Console.WriteLine("Improper input : Usage -> graph \"<function>\"\n\tExample : graph \"math.pow(x,2)\"");
            }
            return fit_to_graph(coordinates_list);
        }
        ...
        {   
            ...
            coordinates_lines.LineTo(new SKPoint(width,height/2));
            coordinates_lines.Close();
            coordinates_lines.MoveTo(new SKPoint(width/2,0));
            coordinates_lines.LineTo(new SKPoint(width/2,height));
            coordinates_lines.Close();
        }
        ...
        public void delete_data()
        {
            string current_directory = Directory.GetCurrentDirectory() + "\\";
            string target_file = "data.json";
            string path = @current_directory + target_file;
